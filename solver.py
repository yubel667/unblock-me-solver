...
def solve(initial_state: BoardState) -> Tuple[Optional[List[Dict]], int, float]:
    ...
    # Priority queue stores (g_score, h_score, state_id, state, path)
    # g_score = distance from start (ensures BFS/optimality)
    # h_score = -state.get_value() (lower is better, used to break ties at same g)
    
    counter = 0
    start_h = -initial_state.get_value()
    # (g, h, id, state, path)
    pq = [(0, start_h, counter, initial_state, [])]
    
    # In BFS, the first time we see a state, it's via the shortest path.
    visited = {initial_state.get_state_identifier()}
    ...
    while pq:
        ...
        for neighbor in curr_state.get_neighbor_states():
            ...
            if state_id not in visited:
                ...
                counter += 1
                heapq.heappush(pq, (new_g, new_h, counter, neighbor, new_path))
